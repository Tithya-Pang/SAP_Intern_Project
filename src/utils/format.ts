...
import type { RequestStatus, RiskLevel, TemporaryCreditRequest } from '@/types/domain';
...
export const formatDate = (value?: string, withTime = false) =>
  value ? dayjs(value).tz(APP_TIMEZONE).format(withTime ? 'DD MMM YYYY, hh:mm A' : 'DD MMM YYYY') : '—';
export const statusLabels: Record<RequestStatus, string> = {
  DRAFT:'Draft', PENDING_SALES_MANAGER_APPROVAL:'Pending Approval', MORE_INFO_REQUIRED:'More Info Required',
  RESUBMITTED:'Resubmitted', APPROVED:'Approved', ACTIVE:'Active', REJECTED:'Rejected',
  DUE_TODAY:'Due Today', OVERDUE:'Overdue', SETTLED:'Settled', CANCELLED:'Cancelled'
};
export const statusColors: Record<RequestStatus, string> = {
  DRAFT:'default', PENDING_SALES_MANAGER_APPROVAL:'orange', MORE_INFO_REQUIRED:'blue', RESUBMITTED:'cyan',
  APPROVED:'green', ACTIVE:'green', REJECTED:'red', DUE_TODAY:'orange', OVERDUE:'red',
  SETTLED:'green', CANCELLED:'default'
};
export const riskColors: Record<RiskLevel, string> = { LOW:'green', MEDIUM:'orange', HIGH:'red' };
